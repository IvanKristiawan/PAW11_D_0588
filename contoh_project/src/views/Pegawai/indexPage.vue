                        ...
                        <table class="table table-striped table-bordered mt4">
                            ...
                            <tbody>
                                <tr v-for="(departemen, id) in departemens" :key="id">
                                    <td>{{ departemen.nomor_induk_pegawai }}</td>
                                    <td>{{ departemen.nama_pegawai }}</td>
                                    <td>{{ departemen.id_departemen }}</td>
                                    <td>{{ departemen.email }}</td>
                                    <td>{{ departemen.telepon }}</td>
                                    <td>{{ departemen.gender }}</td>
                                    <td>{{ departemen.status }}</td>
                                    <td class="text-center">
                                        <router-link :to="{
                                            name:
                                                'pegawai.edit', params: { id: departemen.id }
        ...
        onMounted(() => {
            ...
            axios.get('http://localhost:8000/api/pegawais')
                .then(response => {
                    ...
                    // alert(response.data.data[0].nomor_induk_pegawai)
                }).catch(error => {
